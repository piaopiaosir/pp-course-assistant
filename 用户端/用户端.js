...
// =========================================================================
// 面板布局样式常量 (LAYOUT_CSS)
// 定义悬浮面板的整体外观：卡片、选项卡、按钮、表单、滚动条等自定义样式
// =========================================================================
const LAYOUT_CSS = `
@keyframes pulse {
  0%, 100% { opacity: 1; }
...
`;
// 注入自定义CSS样式：优先使用 GM_addStyle（油猴API），否则用 style 标签降级方案
...
// =========================================================================
// 主 IIFE（立即调用函数表达式）
// 接收外部依赖：Vue3、Pinia、RxJS、MD5、ElementPlus
// 整个脚本的核心逻辑都在此闭包内执行
// =========================================================================
(function (vue, pinia, rxjs, md5, ElementPlus) {
  ...
  // =========================================================================
  // iframe检测：脚本运行在iframe中时，仅保留随堂练习(answerQuestion2)场景
  // =========================================================================  
  ...
  // =========================================================================
  // GM_* API 安全封装：检测API是否存在，不存在则返回 undefined
  // 用于兼容不同脚本管理器（Tampermonkey/Violentmonkey/ScriptCat）
  // =========================================================================  
  ...
  // 规范：访问页面变量(getCookie/uid/FID/param/mArg/top/setInterval/setTimeout/Function等)必须用_unsafeWindow
  //       window.location等标准DOM API在grant模式下通常可用，但为一致性建议统一用_unsafeWindow

  // =========================================================================
  // 正则表达式常量 (REGEX)
  // 用于题目文本清洗、HTML标签过滤、判断题答案判断、URL参数提取等
  // =========================================================================
  const REGEX = {
    CLEAN_TITLE: /^【.*?】\s*|\s*（\d+\.\d+分）$/g,        // 清洗题目标题：去掉 【题型】 前缀和 （分数）后缀
    HTML_TAGS: /<((?!img|sub|sup|br)[^>]+)>/g,           // 过滤HTML标签，保留img/sub/sup/br
    NBSP: /&nbsp;/g,                                         // 替换不间断空格
    WHITESPACE: /\s+/g,                                      // 合并多余空白字符
    BR_TAG: /<br\s*\/?>/g,                                   // 将<br>转换为换行
    IMG_TAG: /<img.*?src="(.*?)".*?>/g,                      // 提取图片URL
    OBJECT_ID: /objectId=([a-f0-9]+)/i,                      // 提取视频/任务objectId
    JOB_ID: /[?&]jobid=([^&]+)/i,                            // 提取任务jobId
    HEX_HASH: /([a-f0-9]{24,})/i,                            // 匹配hex哈希值
    JUDGE_TRUE: /(^|,)(是|对|正确|确定|√|对的|是的|正确的|true|True|T|yes|1)(,|$)/, // 判断答案为"正确"
    JUDGE_FALSE: /(^|,)(非|否|错|错误|×|X|错的|不对|不正确的|不正确|不是|不是的|false|False|F|no|0)(,|$)/ // 判断答案为"错误"
  };

  // =========================================================================
  // DOM选择器常量 (SELECTORS)
  // 超星(CX)平台和知到(ZHS)平台的题目、选项容器选择器
  // =========================================================================
  const SELECTORS = {
    CX_VIDEO: 'video',                                          // 超星视频元素
    CX_AUDIO: 'audio',                                          // 超星音频元素
    CX_QUESTION_ZJ: '.TiMu',                                    // 超星章节/作业题目容器
    CX_QUESTION_ZY_KS: '.questionLi',                           // 超星测验/考试题目容器
    CX_OPTION_ZJ: '[class*="before-after"]',                   // 超星章节/作业选项
    CX_OPTION_ZY_KS: '.answerBg',                              // 超星测验/考试选项
    ZHS_QUESTION: '.examPaper_subject',                         // 知到题目容器
    ZHS_OPTION: '.subject_node .nodeLab'                       // 知到选项
  };
  ...
  // 获取赞助链接HTML（从服务端响应中获取url，避免硬编码和缓存）
  ...
  // =========================================================================
  // AI模型配置管理
  // 从服务端动态获取AI模型列表、默认模型、费用等配置信息
  // =========================================================================
  
  let globalModelConfig = null;  // 全局AI模型配置缓存
  let globalPollInterval = 1000; // 全局轮询间隔（启动时从服务端获取，默认1秒）
  
  // 从服务器获取AI模型配置（模型列表、默认值、费用）
  ...
  // 带随机抖动的延迟：在 baseSecond ± jitter 范围内随机
  ...
  // 从 document.cookie 中读取指定名称的cookie值
  const getCookie = (name) => {
    const match = document.cookie.match(new RegExp('(^| )' + name + '=([^;]+)'));
    return match ? match[2] : '';
  };

  // 获取用户UID（优先从 unsafeWindow 获取，然后从cookie获取）
  let _cachedUid = null;
  const getUid = () => {
  ...
  // 实时获取用户UID（不走缓存，每次从页面cookie重新读取，防止篡改缓存值冒充他人）
  ...
  const formatDateTime = (dt) => `${pad(dt.getHours())}:${pad(dt.getMinutes())}:${pad(dt.getSeconds())}`; // HH:MM:SS
  const getDateTime = () => formatDateTime(new Date()); // 获取当前时间字符串
  const formatDuration = (seconds) => `${Math.floor(seconds / 60).toString().padStart(2, '0')}:${Math.floor(seconds % 60).toString().padStart(2, '0')}`; // MM:SS

  // 获取当前随机选中的服务器URL
  const getRandomServer = () => CURRENT_SERVER;

  // 服务器故障转移：当前服务器不可用时切换到备用服务器
  ...
  // =========================================================================
  // Pinia 配置存储 (configStore)
  // 管理脚本的所有配置项：平台参数、答题模式、服务器Token、面板位置等
  // 配置通过 GM_setValue 持久化存储，初始化时从存储中恢复
  // =========================================================================
  ...
  // =========================================================================
  // Pinia 题目存储 (questionStore)
  // 管理已解析和已回答的题目列表，用于答题记录展示
  // =========================================================================
  ...
  // =========================================================================
  // Pinia 进度存储 (progressStore)
  // 管理当前任务进度信息：任务名、完成百分比、当前/总时长、播放状态等
  // =========================================================================
  ...
  // 运行状态卡片独立组件 - 拥有独立渲染作用域，progressStore变化只触发自身重渲染
    ...
    setup(__props) {
      ...
      return (_ctx, _cache) => {
        ...
        return vue.openBlock(), vue.createElementBlock("div", { style: { "padding": "4px" } }, [
          ...
          // 运行状态卡片 - 独立组件，progressStore变化只触发自身重渲染
  ...
  // =========================================================================
  // Vue SFC导出工具 (_export_sfc)
  // 为Vue组件注入props和作用域ID，模拟SFC编译结果
  // =========================================================================
  ...
  const _sfc_main$7 =  vue.defineComponent({
    ...
    setup(__props) {
      ...
      // UI更新触发器：当非响应式数据（如config params的getter）变化时，
      // 调用triggerUiUpdate()通知Vue重新计算依赖这些数据的computed属性
  ...
  // =========================================================================
  // iframe扫描器 (FrameScanner)
  // collectDirect: 直接子iframe, collectDeep: 递归深层扫描所有iframe
  // =========================================================================
  const FrameScanner = {
    collectDirect(root) {
  ...
  var Font_1 = Font;
  // =========================================================================
  // 超星加密字体解密 (decodeCipherFont)
  // 解析页面中 .font-cxsecret 的加密字体，通过TFF字体文件映射表还原真实文字
  // 工作流程：1.查找font-cxsecret样式 2.解析base64字体 3.用MD5签名匹配码表 4.替换加密字符
  // =========================================================================
  ...
  // =========================================================================
  // 题目处理器基类 (QuestionProcessor)
  // 定义题目解析、答案填充、已答判断等核心抽象方法
  // 子类：CxQuestionHandler(超星), ZhsQuestionHandler(知到)
  // =========================================================================
  class QuestionProcessor {
    constructor() {
      __publicField(this, "_document", document);           // 操作的文档对象
      __publicField(this, "_window", _unsafeWindow);         // 页面window对象
      __publicField(this, "addLog");                          // 日志记录函数
      __publicField(this, "addQuestion");                     // 题目记录函数
      __publicField(this, "questions", []);                   // 解析出的题目列表
      __publicField(this, "correctNum", 0);                   // 正确答题数量
      __publicField(this, "isFilling", false);                // 是否正在填充答案
      __publicField(this, "parseHtml", () => {                // 抽象方法：解析HTML提取题目
        throw new Error("Abstract method: parseHtml must be implemented by subclass");
      });
      __publicField(this, "fillQuestion", (question) => {     // 抽象方法：为题目填充答案
        throw new Error("Abstract method: fillQuestion must be implemented by subclass");
      });
      
      // 检查题目是否已被作答
      __publicField(this, "checkIfAnswered", (question) => {
        // 单选题(0)和多选题(1)：检查选项的选中状态
        if (question.type === "0" || question.type === "1") {
          for (const key in question.options) {
            const optEl = question.options[key];
            // 1. aria-checked
            if (optEl.getAttribute("aria-checked") === "true") return true;
            // 2. li元素自身的cur/selected类
            if (optEl.classList.contains("cur") || optEl.classList.contains("selected")) return true;
            // 3. 内部span的cur/selected类
      ...
      // 去除HTML标签，保留img/sub/sup/br，将<br>转换为\n
  ...
  // =========================================================================
  // 服务端公告对话框 (showNoticeDialog)
  // 以模态弹窗展示服务端公告，支持加入QQ群或关闭
  // =========================================================================
  ...
  // =========================================================================
  // 检查服务端公告 (checkServerNotice)
  // 向服务器请求最新公告，如果用户之前没有关闭过则显示公告弹窗
  // =========================================================================
  ...
  // =========================================================================
  // 核心：获取题目答案 (fetchAnswerData)
  // 根据配置的答题模式（正常/校验/AI），向服务器请求题目答案
  // 支持异步队列轮询：服务端返回taskId后，客户端定时轮询查询结果
  // 流程：构建题目数据 -> 确定答题模式 -> 发送请求 -> 轮询结果 -> 校验失败则AI深度思考
  // =========================================================================
  const fetchAnswerData = async (question) => {
    ...
    let useAIOnly = false;  
    let enableWebSearch = false;  // 联网搜索开关
      ...
      return new Promise((resolve) => {
        ...
        _GM_xmlhttpRequest({
          ...
          data: JSON.stringify({
            ...
            aiMode: useAIOnly,  
            enableWebSearch: enableWebSearch,  // 联网搜索开关
            model: modelType,  
            checkOnly: checkOnly,  
  ...
  // =========================================================================
  // 正确答案检测与上报 (detectAndReportResults)
  // 在提交后检测页面的标记元素(marking_dui/marking_cuo)来判断答案正确性
  // 并将正确性结果上报到服务器用于优化题库
  // =========================================================================
  ...
  // =========================================================================
  // 答案正确性上报 (reportAnswerResults)
  // 将答题结果的正确/错误状态上报到服务器，用于优化题库数据
  // =========================================================================
  ...
  // =========================================================================
  // 莱文斯坦距离相似度计算 (computeMatch)
  // 使用编辑距离（Levenshtein Distance）算法计算两个字符串的相似度(0-100)
  // =========================================================================
  ...
  // =========================================================================
  // 最佳选项匹配 (pickBestOption)
  // 在选项列表中通过相似度算法找到与答案最匹配的选项
  // 完全匹配相似度=100直接返回，否则返回第一个超过阈值的选项
  // =========================================================================
  ...
  // =========================================================================
  // 超星平台答题处理器 (CxQuestionHandler extends QuestionProcessor)
  // 负责超星学习通的题目解析、答案查询、自动填写
  // 支持章节/作业(zj)、测验(zy)、考试(ks)三种题型模式
  // =========================================================================
  class CxQuestionHandler extends QuestionProcessor {
    constructor(type, iframe) {
      super();
      __publicField(this, "type");              // 题目类型：zj(章节/作业)、zy(测验)、ks(考试)
      // 初始化：解析题目 -> 逐题查询答案 -> 填写答案
      ...
      // 为题目填充答案：根据题型（单选/多选/填空/判断/简答/排序）执行不同的填充逻辑
        ...
        try {
          ...
          for (const key in question.options) {
            if (["zj", "zy"].includes(this.type)) {
              ...
              // 判断答案是否匹配当前选项：通过aria-label或选项文本内容
              let isMatch = false;
              if (answer === "true") {
              ...
              if (isMatch) {
                ...
                // 4. 内部是否有onChecked/check_answer标记
          ...
          // 连线题：服务端已返回纯字母数组 ["C", "D", "A"]，按顺序对应select
          const selectElements = question.element.querySelectorAll("select.dept_select");
        ...
        if (["zy", "ks"].includes(this.type)) {
          ...
          if (readComprehensionItems.length > 0) {
            ...
            readComprehensionItems.forEach((subItem) => {
              ...
              // 子题选项：根据childType选择不同的选择器
              // 判断题(childType=3)的li有before-after class，用CX_OPTION_ZJ
              // 单选题(childType=0)/多选题(childType=1)的li没有before-after class，用ul选择器
              // 填空题(childType=2)没有选项
              let subOptionElements;
              let optionSelector = ".fl.after";
              if (childType === "3") {
                // 判断题：li有before-after class
                subOptionElements = subItem.querySelectorAll(SELECTORS.CX_OPTION_ZJ);
              } else if (childType === "0" || childType === "1") {
                // 单选/多选：li没有before-after class
            ...
            return; // 复合题拆解完毕，不添加整体大题
          }
          
          // 检查是否为连线题（题型文本包含"连线" 或 .matching结构 或 data=11）
          ...
          if (isMatchingQuestion) {
            // 连线题：把完整信息（左侧题目+右侧选项）都放进题目，交给AI处理
  ...
  // =========================================================================
  // 超星章节学习主逻辑 (useCxChapterLogic)
  // 核心驱动函数：监控iframe加载、识别任务类型（视频/音频/作业/PPT/电子书）
  // 根据配置的播放模式（正常/模拟播放）和答题模式执行自动化
  // =========================================================================
  const useCxChapterLogic = () => {
    ...
    // =========================================================================
    // 欺骗浏览器播放 (FakeMediaPlayer)
    // 创建一个静音的音频循环播放，欺骗浏览器认为页面正在播放媒体
    // 防止浏览器后台时限制性能或冻结定时器
    // =========================================================================
    const FakeMediaPlayer = (() => {
      ...
      const start = () => {
        if (isPlaying) return;
        try {
          // 方法1: 使用 Web Audio API 创建静音音频上下文
    ...
    // =========================================================================
    // 模拟播放视频/音频 (simulateVideoPlay)
    // 不上传真实播放数据，而是按配置倍速模拟播放进度，定时上报完成状态
    // 从页面脚本中提取attachment数据获取classId/jobId/objectId/dtoken等信息
    // =========================================================================
    ...
    // =========================================================================
    // 直接播放视频/音频 (playMediaDirectly)
    // 真实播放媒体内容，静音并设置倍速，等待播放结束
    // 可选处理视频内嵌的答题弹窗
    // =========================================================================
        ...
        const playLoopCallback = async () => {
          ...
          if (mediaElement && !isExecuted) {
            ...
            const listener = async () => {
              // 挂机模式使用 BackgroundWorker 延迟，避免最小化时节流
              if (afkEnabledPlay) {
  ...
  // =========================================================================
  // 触发DOM元素点击事件 (clickOption)
  // 通过 focus + mousedown + mouseup + click 完整模拟用户点击
  // =========================================================================
  ...
  // =========================================================================
  // 超星随堂练习逻辑 (useStuActiveLogic)
  // 在随堂练习(answerQuestion2)的iframe中运行
  // 流程：关闭主面板 -> 等待Vue渲染题目 -> 解析题目 -> 查询答案 -> 自动填写
  // =========================================================================
  const useStuActiveLogic = async () => {
    // 通知父窗口关闭面板（iframe中不需要显示脚本面板）
    try {
  ...
  // =========================================================================
  // 超星作业逻辑 (useCxWorkLogic)
  // 处理作业和测验页面，解析题目并自动填写答案
  // =========================================================================
  ...
  // =========================================================================
  // 超星新版考试逻辑 (useCxExamLogic)
  // 考试页面：解析题目 -> 填入答案 -> 可选自动切换到下一题
  // =========================================================================
  ...
  // =========================================================================
  // 知到智慧树平台答题处理器 (ZhsQuestionHandler extends QuestionProcessor)
  // 适配智慧树的题目解析、已答判定、答案填充（单选/多选/判断/填空/简答）
  // =========================================================================
  ...
  // =========================================================================
  // 错误钩子 (hookError)
  // 拦截 setInterval/setTimeout 调用，过滤掉智慧树平台的 "checkoutNotTrustScript" 检测代码
  // =========================================================================
  const hookError = () => {
  ...
  // =========================================================================
  // XMLHttpRequest拦截器 (XMLHttpRequestInterceptor)
  // 拦截指定的XHR请求URL，在请求完成时触发回调
  // 用于知到平台在特定API请求完成后注入答题逻辑
  // =========================================================================
  ...
  // =========================================================================
  // 知到智慧树答题逻辑 (useZhsAnswerLogic)
  // 通过XMLHttpRequest拦截器监听API请求，在答题页面加载完成后自动答题
  // =========================================================================
  ...
  const _sfc_main$3 =  vue.defineComponent({
    ...
    setup(__props, { emit: __emit }) {
      ...
      // =========================================================================
      // URL路由匹配表
      // 根据当前页面URL匹配对应的处理逻辑函数
      // =========================================================================
      const urlLogicPairs = [
        { keyword: "/mycourse/studentstudy", logic: useCxChapterLogic },   // 章节学习页面
        { keyword: "/mooc2/work/dowork", logic: useCxWorkLogic },          // 作业/测验页面
        { keyword: "/exam-ans/", logic: useCxExamLogic },                  // 新版考试页面
        { keyword: "/work/index", logic: useCxWorkLogic },                 // 旧版作业首页
        { keyword: "/work/doTest", logic: useCxWorkLogic },                // 旧版测验页面
        { keyword: "/work/calcAnswer", logic: useCxWorkLogic },            // 旧版计算答案
        { keyword: "/knowledge/start", logic: useCxChapterLogic },         // 章节知识页面
        {
        ...
        { keyword: "/stuExamWeb.html", logic: useZhsAnswerLogic },         // 知到考试页面
        { keyword: "answerQuestion2", logic: useStuActiveLogic }          // 随堂练习
      ];
    ...
    setup(__props) {
      ...
      const forceRepaint = () => {
        vue.nextTick(() => {
          ...
          if (parent) {
            // 在 el-card 上切换 translateZ，迫使浏览器的合成层重新上传纹理
            // 解决父容器 CSS 过渡期间子节点 display:none 不被重绘的问题
  ...
  // =========================================================================
  // Webpack Hook (hookWebpack)
  // 拦截 Function.prototype.call，在知到平台Vue（v2.5.0）实例化时注入 __Ivue__ 属性
  // 使得可以通过 $el.__Ivue__ 访问Vue组件实例，便于读取题目数据
  // =========================================================================
  const hookWebpack = () => {
  ...
  // =========================================================================
  // Vue应用初始化
  // 等待页面完全加载完成（readyState === "complete"）后再初始化
  // 确保 iframe 等资源已加载，避免状态检测不同步导致iframe src变化时无法触发重新扫描
  // =========================================================================
